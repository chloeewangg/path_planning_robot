...
class ROSNode(Node):
    ...
    def cb_timer(self):
        ...
        if self.shared.acquire():
            posx  = self.shared.robot_pos.x
            posy  = self.shared.robot_pos.y
            head  = self.shared.robot_pos.heading
            self.shared.release()
    ...
    def cb_explore(self, msg):
        ...
        if self.shared.acquire():
            self.shared.cmd = Command.EXPLORE
            self.shared.release()
    ...
    def cb_fetch(self, msg):
        ...
        if self.shared.acquire():
            self.shared.prize = int(prize)
            self.shared.cmd = Command.FETCH
            self.shared.release()
    ...
    def cb_dicts(self, msg):
        ...
        inter_prize_distance_dict = {
            int(key): {
                int(prize_key): raw_dist_dict[key][prize_key]['distance']
        ...
        # Place in shared data!
        if self.shared.acquire():
            self.shared.prize_dist_dict = inter_prize_distance_dict
...
if __name__ == "__main__":
    ...
    rosthread = threading.Thread(
        name="ROSThread", target=run_ros, args=(shared,))
    rosthread.start()
